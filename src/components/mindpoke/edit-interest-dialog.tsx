...
import { X, Plus, Terminal, Trash2, AlertTriangle } from "lucide-react";
  ...
  return (
    <>
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent className="bg-[#0a0a0f] border border-[#ff4444]/50 max-w-md">
          {/* ASCII corners - red themed */}
          <span className="absolute top-0 left-0 font-terminal text-[#ff4444]/50 text-xs">┌</span>
          <span className="absolute top-0 right-0 font-terminal text-[#ff4444]/50 text-xs">┐</span>
          <span className="absolute bottom-0 left-0 font-terminal text-[#ff4444]/50 text-xs">└</span>
          <span className="absolute bottom-0 right-0 font-terminal text-[#ff4444]/50 text-xs">┘</span>

          <DialogHeader className="border-b border-[#ff4444]/30 pb-4">
            <div className="font-terminal text-[10px] text-[#ff4444] mb-2">
              <AlertTriangle className="w-3 h-3 inline mr-1" />
              $ SYSTEM_WARNING :: DESTRUCTIVE_OPERATION
            </div>
            <DialogTitle className="font-serif text-xl text-[#ff4444]">
              Delete Interest?
            </DialogTitle>
            <DialogDescription className="font-terminal text-xs text-[#888888]">
              CONFIRM_DELETION :: This action cannot be undone
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="bg-[#ff4444]/10 border border-[#ff4444]/30 p-4">
              <p className="font-terminal text-sm text-[#e6e6e6] mb-2">
                You are about to delete <span className="text-[#ff4444]">&quot;{interest?.name}&quot;</span>
              </p>
              
              {discoveryCount > 0 && (
                <div className="mt-3 pt-3 border-t border-[#ff4444]/20">
                  <div className="flex items-center gap-2 mb-2">
                    <AlertTriangle className="w-4 h-4 text-[#ffb000]" />
                    <span className="font-terminal text-xs text-[#ffb000]">
                      DATA_LOSS_WARNING
                    </span>
                  </div>
                  <p className="font-terminal text-xs text-[#888888]">
                    <span className="text-[#ffb000] font-bold">{discoveryCount}</span> {discoveryCount === 1 ? 'discovery' : 'discoveries'} linked to this interest will be orphaned and may lose context.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button
                ...
                disabled={isDeleting}
                className="bg-[#ff4444] text-[#0a0a0f] border border-[#ff4444] hover:bg-[#ff4444]/80 font-terminal text-xs"
              >
                {isDeleting ? (
                  "DELETING..."
                ) : (
